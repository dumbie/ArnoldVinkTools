    ...
    public partial class AppLaunchCheck
    {
        public static void Application_LaunchCheck(string ApplicationName, string ProcessName, ProcessPriorityClass ProcessPriority, bool skipFileCheck)
        {
            try
            {
                ...
                if (!skipFileCheck)
                {
                    string[] ApplicationFiles = { "ArnoldVinkTools.exe", "ArnoldVinkTools.exe.Config", "ArnoldVinkTools-Admin.exe", "ArnoldVinkTools-Admin.exe.Config", "Updater.exe", "Updater.exe.Config" };
